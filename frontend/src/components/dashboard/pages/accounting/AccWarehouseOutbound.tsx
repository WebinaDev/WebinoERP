  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-lg font-semibold">{t('auto.accounting_AccWarehouseOutbound.s_c7469826')}</h2>
        <Button size="sm" onClick={openCreate}>
          <Plus className="ms-1 h-4 w-4" /> {t('auto.accounting_AccWarehouseOutbound.s_7cccd716')}
        </Button>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <Select value={filterStatus} onValueChange={(v) => setFilterStatus(v as 'all' | 'draft' | 'posted')}>
          <SelectTrigger className="w-40">
            <SelectValue placeholder={t('auto.accounting_AccWarehouseOutbound.s_55518965')} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{t('auto.accounting_AccWarehouseOutbound.s_7552ab35')}</SelectItem>
            <SelectItem value="draft">{t('auto.accounting_AccWarehouseOutbound.s_7d739ea1')}</SelectItem>
            <SelectItem value="posted">{t('auto.accounting_AccWarehouseOutbound.s_1223f269')}</SelectItem>
          </SelectContent>
        </Select>
        <Select value={filterWarehouseId} onValueChange={setFilterWarehouseId}>
          <SelectTrigger className="w-56">
            <SelectValue placeholder={t('auto.accounting_AccWarehouseOutbound.s_3aeb36b5')} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{t('auto.accounting_AccWarehouseOutbound.s_0a7f24b0')}</SelectItem>
            {warehouses.map((w) => (
      ...
      {loading && <p className="text-sm text-muted-foreground">{t('auto.accounting_AccWarehouseOutbound.s_51617f69')}</p>}

      <div className="overflow-x-auto rounded-md border">
        <table className="w-full min-w-[600px] text-sm">
          <thead>
            <tr className="border-b bg-muted/40">
              <th className="px-3 py-2 text-start font-medium">{t('auto.accounting_AccWarehouseOutbound.s_acc84041')}</th>
              <th className="px-3 py-2 text-start font-medium">{t('auto.accounting_AccWarehouseOutbound.s_3aeb36b5')}</th>
              <th className="px-3 py-2 text-start font-medium">{t('auto.accounting_AccWarehouseOutbound.s_2c09d41c')}</th>
              <th className="px-3 py-2 text-start font-medium">{t('auto.accounting_AccWarehouseOutbound.s_55518965')}</th>
              <th className="px-3 py-2 text-start font-medium">{t('auto.accounting_AccWarehouseOutbound.s_6a67beb8')}</th>
              <th className="px-3 py-2 text-start font-medium">{t('auto.accounting_AccWarehouseOutbound.s_8d1cc546')}</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((doc) => (
                ...
                <tr className="border-b border-border/60">
                  ...
                  <td className="px-3 py-1.5">
                    <Badge variant={doc.status === 'posted' ? 'default' : 'secondary'}>
                      {doc.status === 'posted' ? t('auto.accounting_AccWarehouseOutbound.s_1223f269') : t('auto.accounting_AccWarehouseOutbound.s_7d739ea1')}
                    </Badge>
                  </td>
                    ...
                    <div className="flex gap-1">
                      {doc.status === 'draft' && (
                        <Button variant="outline" size="sm" onClick={() => void handlePost(doc.id)}>
                          {t('auto.accounting_AccWarehouseOutbound.s_0c801ba8')}
                        </Button>
                      )}
                      <Button
                        variant="ghost"
                  ...
                  <tr className="border-b border-border/60 bg-muted/20">
                    <td colSpan={6} className="px-6 py-3">
                      {doc.items && doc.items.length > 0 ? (
                        <>
                          <p className="mb-1.5 text-xs font-medium text-muted-foreground">{t('auto.accounting_AccWarehouseOutbound.s_16d8617a')}</p>
                          <table className="w-full text-xs">
                            <thead>
                              <tr>
                                <th className="px-2 py-1 text-start font-medium">{t('auto.accounting_AccWarehouseOutbound.s_9865e1d5')}</th>
                                <th className="px-2 py-1 text-start font-medium">{t('auto.accounting_AccWarehouseOutbound.s_687f8df3')}</th>
                                <th className="px-2 py-1 text-start font-medium">{t('auto.accounting_AccWarehouseOutbound.s_3709c4a6')}</th>
                              </tr>
                            </thead>
                            <tbody>
                              {doc.items.map((it, i) => (
                        ...
                        <p className="text-xs text-muted-foreground">{t('auto.accounting_AccWarehouseOutbound.s_8b121c6a')}</p>
                      )}
                    </td>
                  </tr>
                )}
            ...
            {!loading && rows.length === 0 && (
              <tr>
                <td colSpan={6} className="px-3 py-6 text-center text-muted-foreground">
                  {t('auto.accounting_AccWarehouseOutbound.s_a8a405d5')}
                </td>
              </tr>
            )}
          </tbody>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t('auto.accounting_AccWarehouseOutbound.s_beb81a37')}</DialogTitle>
            <DialogDescription>{t('auto.accounting_AccWarehouseOutbound.s_9a00e161')}</DialogDescription>
          </DialogHeader>
          ...
          <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950/50 dark:text-amber-100">
            {t('auto.accounting_AccWarehouseOutbound.s_dfe85627')}
          </div>

          <div className="space-y-3">
            <div className="space-y-1">
              <label className="text-sm font-medium">{t('auto.accounting_AccWarehouseOutbound.s_3aeb36b5')}</label>
              <Select value={formWarehouseId} onValueChange={setFormWarehouseId}>
                <SelectTrigger><SelectValue placeholder={t('auto.accounting_AccWarehouseOutbound.s_d3c3902f')} /></SelectTrigger>
                <SelectContent>
                  {warehouses.map((w) => (
                    <SelectItem key={w.id} value={String(w.id)}>{w.name}</SelectItem>
                  ))}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-sm font-medium">{t('auto.accounting_AccWarehouseOutbound.s_101ca9b8')}</label>
                <Button type="button" variant="outline" size="sm" onClick={addItem}>
                  <Plus className="ms-1 h-3.5 w-3.5" /> {t('auto.accounting_AccWarehouseOutbound.s_ffd73187')}
                </Button>
              </div>
              {formItems.map((item, idx) => (
                <div key={idx} className="flex items-end gap-2 rounded-md border p-2">
                  <div className="flex-1 space-y-1">
                    <label className="text-xs text-muted-foreground">{t('auto.accounting_AccWarehouseOutbound.s_9865e1d5')}</label>
                    <Input value={item.product_id} onChange={(e) => updateItem(idx, 'product_id', e.target.value)} />
                  </div>
                  <div className="w-24 space-y-1">
                    <label className="text-xs text-muted-foreground">{t('auto.accounting_AccWarehouseOutbound.s_687f8df3')}</label>
                    <Input value={item.quantity} onChange={(e) => updateItem(idx, 'quantity', e.target.value)} />
                  </div>
                  <div className="w-28 space-y-1">
                    <label className="text-xs text-muted-foreground">{t('auto.accounting_AccWarehouseOutbound.s_3709c4a6')}</label>
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium">{t('auto.accounting_AccWarehouseOutbound.s_2c09d41c')}</label>
          ...
          <DialogFooter>
            <Button onClick={handleCreate} disabled={saving}>
              {saving ? t('auto.accounting_AccWarehouseOutbound.s_4b7554d6') : t('auto.accounting_AccWarehouseOutbound.s_f14ef897')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
