...
export function SiteProvisionWizardPage() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        if (platformServers[0]?.id) setServerId(platformServers[0].id);
  ...
  const persistWizard = useCallback(async () => {
    ...
    selectedFeatures, categoryId, typeId, siteTypeSlug, serverId, provision?.id,
  ]);
      ...
      {step === 4 ? (
        <Card>
          <CardContent className="grid gap-3 pt-6">
            ...
            <Button disabled={!siteName || !serverId || pending} onClick={() => void nextStep()}>{t('continue')}</Button>
          </CardContent>
        </Card>
      ) : null}
