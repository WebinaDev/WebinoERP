...
function powerTone(power?: string): string {
  switch (power) {
    case 'running':
      return 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border-emerald-500/30';
    case 'stopped':
...
function canPower(row: SiteProvision): boolean {
  return !['draft', 'pending', 'provisioning', 'cancelled'].includes(row.status);
}
...
export function SiteProvisionsListPage() {
  ...
  const load = useCallback(async (nextPage = page) => {
    setError(null);
    try {
      const result = await fetchProvisions(nextPage, 20);
      setRows(result.data);
  ...
  async function power(id: number, action: 'start' | 'stop') {
    if (action === 'stop' && !window.confirm(t('controlConfirmStop'))) return;
    setBusyId(id);
    setError(null);
    try {
  ...
  return (
    <div className="space-y-6" data-testid="sites-fleet">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-xs">
            {t('fleetStats', { total, ready: readyCount })}
          </p>
        </div>
            ...
            >
              ...
              <CardContent className="relative space-y-3 text-sm">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  ...
                  {row.status === 'ready' || row.status === 'ssl_pending' ? (
                  ...
                  <Button
                    ...
                    data-testid={`site-destroy-${row.id}`}
                  >
      ...
      {rows.length === 0 && !error ? (
        <p className="text-muted-foreground text-sm">{t('emptyProvisions')}</p>
      ) : null}
    </div>
  );
}
