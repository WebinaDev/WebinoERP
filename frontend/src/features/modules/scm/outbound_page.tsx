...
import { createOutbound, listWarehouses, postOutbound } from '@/lib/api/scm';
...
export function OutboundPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const res = await apiClient.get('/v1/scm/outbound', { params });
  ...
  return (
    <CrmPageLayout
      title={tNav('nav.erp.scm.outbound')}
      actions={<Button onClick={openCreate}><Plus className="ms-1 h-4 w-4" />{t('createOutbound')}</Button>}
      {...layoutProps}
    >
      ...
      <div className="mb-3 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950/50 dark:text-amber-100">
        {t('outboundWarn')}
      </div>
                    ...
                    <div className="flex gap-1">
                      {r.status !== 'posted' && (
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => void postOutbound(r.id).then(() => { setSuccess(t('postOutbound')); void load(); }).catch(applyAxiosError)}
                        >
                          {t('postOutbound')}
                        </Button>
                      )}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] max-w-xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t('createOutbound')}</DialogTitle>
            <DialogDescription>{t('outboundHint')}</DialogDescription>
          </DialogHeader>
